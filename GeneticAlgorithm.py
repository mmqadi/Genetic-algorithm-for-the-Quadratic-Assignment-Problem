...
class geneticAlgorithmForQAP:
    ...
    def runGeneticAlgorithm(self, filename, iteration):
        ...
        OUTPUT_FILE = "./results/" + filename + "_" + str(iteration) + ".sol"
        f = open(OUTPUT_FILE, "w+")
        b = open("./iterations/" + filename + "_" + str(iteration), "w+")

        for t in range(self._numberOfIterations):
            b.write(str(bestSolutionFitness) + " ")
            if (t % 200 == 0):
...
fileToRead = open("instances/tai30a", 'r')
dim = int(fileToRead.readline())
counter = 0
dist = []
while (counter < dim):
...
if __name__ == "__main__":
    filename = input("Enter the name of the instance: ")

    GA = geneticAlgorithmForQAP(100, 0.1, 1000, dim, weight, dist)
    for i in range(10):
        print("Starting trail " + str(i + 1) + ".")
